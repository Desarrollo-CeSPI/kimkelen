 ...
 * @author ncuesta
 */   
class BbaCourseSubjectNotAverageableMarksForm extends CourseSubjectNotAverageableMarksForm
{
  public function configure()
  {
    ...
    foreach ($this->object->getCourseSubjectStudentsNotAverageable() as $course_subject_student)
    {
      ...
      $widgets[$calification_final_widget_name] = new sfWidgetFormSelect(array(
          'choices'  => BaseCustomOptionsHolder::getInstance('BbaNotAverageableCalificationType')->getOptions(true)
           ));
     $validators[$calification_final_widget_name] = new sfValidatorChoice(array(
        'choices' => BaseCustomOptionsHolder::getInstance('BbaNotAverageableCalificationType')->getKeys(),
        'required'=>false));
  ...
  protected function doSave($con = null)
  {
    ...
    foreach ($this->object->getCourseSubjectStudentsNotAverageable() as $course_subject_student)
    {
      ...
      $array= array(1,7,8,9,10);
      $array_marks = array(7,8,9,10);
        ...
        {//aprobado
            ...
            if(!is_null($sdcs) || is_null($sacs))
            {
                ...
                if (in_array($value,$array_marks) )
                {
                     $student_approved_course_subject->setMark($value);
                }
