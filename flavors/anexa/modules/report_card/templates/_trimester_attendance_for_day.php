  ...
  <table class="lefttable">
    ...
    <tr>
      ...
      <?php foreach ($periods as $period): ?>
        ...
        <?php $absences_j = $student->getTotalAbsences($division->getCareerSchoolYearId(), $period, null, false); ?>
        <?php if ($period->getIsClosed()): ?>
          <td><?php echo round($absences_j, 2) ?></td>
          <?php $total += $absences ?>
          <?php $total_j += $absences_j ?>
        <?php else: ?>
          <td></td>
        <?php endif; ?>

      <?php endforeach; ?>
      <td><?php echo round($total_j, 2);?></td>
      <td><?php echo round($total, 2);?></td>
      <td><?php echo round($total_j - $total, 2);?></td>
    </tr>
  </table>
</div>
