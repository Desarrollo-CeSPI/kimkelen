...
var dcWidgetAjaxDependence={
  ...
  updateDependencies: function (id)
  {
    this.getDependenciesFor(id).each(function(e){
      if ( ($(e.id) != null) && (dcWidgetAjaxDependence.initialLoading==0) )
      {
        $(e.id).disabled=true;
      }
      if ( e.observe_id==id )
      {
        var obj=dcWidgetAjaxDependence.widgets.get(e.id).toObject();
        Event.observe(obj.observe_id,obj.event_type,function(event){
          dcWidgetAjaxDependence.ajaxUpdate(obj.id, function () {
            dcWidgetAjaxDependence.updateDependencies(obj.id);
          });
        });
      }
    });  
    if (dcWidgetAjaxDependence.initialLoading > 0)
    {
      dcWidgetAjaxDependence.initialLoading--;
    }
  },
  ...
  addToForest: function (id,observe_id)
  {
    ...
    t=dcWidgetAjaxDependence.forest.find(function(tr){ 
      return tr.find(function (e){
  ...
  {
    dcWidgetAjaxDependence.initialLoading=0;
    dcWidgetAjaxDependence.forest.each(function (t){
      dcWidgetAjaxDependence.initialLoading+=t.length();
    });
    dcWidgetAjaxDependence.forest.each(function(t){
      dcWidgetAjaxDependence.updateTreeOnLoad(t);
    });
    dcWidgetAjaxDependence.widgets.each(function (pair){
      var obj=pair.value.toObject();
      var id=pair.key;
      if ( $(obj.observe_id)!=null )
      {
        Event.observe(obj.observe_id,obj.event_type,function(){
          dcWidgetAjaxDependence.ajaxUpdate(id, function () {
            dcWidgetAjaxDependence.updateDependencies(id);
          });
        });
      }
    });
  ...
  updateTreeOnLoad: function (tree)
  {
    return this.ajaxUpdate(tree.getNode().id, function (){
      tree.getChildren().each(function(c){
        dcWidgetAjaxDependence.updateTreeOnLoad(c);
      });
    });
  },
