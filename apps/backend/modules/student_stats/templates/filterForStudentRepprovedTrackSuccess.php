...
<div id="sf_admin_container">
  <h1>
    <?php echo __('Choose values for repproved track')?>
  </h1>
  <div id="sf_admin_content">

    <form method="POST" action="<?php echo url_for('student_stats/filterForStudentRepprovedTrack') ?>">
      <?php echo strtr($form->getWidgetSchema()->getFormFormatter()->getDecoratorFormat(), array("%content%" => (string) $form)) ?>
      <ul class="sf_admin_actions">
        <li><input type="submit" value="<?php echo __('Generate track') ?>" /></li>
      </ul>
    </form>
  </div>
</div>