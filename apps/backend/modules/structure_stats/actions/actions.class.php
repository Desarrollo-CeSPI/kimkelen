 ...
 * @subpackage structure_stats
 * @author     Your name here
 * @version    SVN: $Id: actions.class.php 12479 2008-10-31 10:54:40Z fabien $
 */
class structure_statsActions extends sfActions
{
 public function executeFilterForStructureStats(sfWebRequest $request)
  {
    $this->form = new StructureReportsForm();
    if ($request->isMethod('POST'))
    {
      $this->form->bind($request->getParameter($this->form->getName()));
      if ($this->form->isValid())
      {
        $this->getUser()->setReferenceFor($this, 'structure_stats');
        $this->forward('structure_stats', 'structureReports');
      }
    }
  }
  ...
  public function executeSetStructureFilters(sfWebRequest $request)
  {
    $parameters = $request->getGetParameters();
    $this->getUser()->setAttribute('structure_stats.filters', array(), 'admin_module');
    $this->getUser()->setAttribute('structure_stats.filters', $parameters, 'admin_module');
    $this->getUser()->setAttribute('career_school_year', $parameters['career_school_year']);

    $this->redirect('@structure_stats');
  }
  ...
  public function executeStructureReports(sfWebRequest $request)
  {
    $params = $request->getParameter('structure_reports');
    unset($params['_csrf_token']);

    $this->generateStructureReports($params);
