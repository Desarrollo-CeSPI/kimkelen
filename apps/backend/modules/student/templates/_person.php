...
<h2><?php echo __('Personal data') ?> </h2>
  ...
  <div class="sf_admin_form_row sf_admin_Text sf_admin_form_field_cuil">
    ...
    <label for="cuil"> <?php echo __('Cuil');?> </label>
      <?php echo (is_null($student->getPerson()->getCuil()) | $student->getPerson()->getCuil() == '') ? 'No ha sido cargado' : $student->getPerson()->getCuil();?>
    </div>
    <div style="margin-top: 1px; clear: both;"></div>
  </div>
  ...
  <div class="sf_admin_form_row sf_admin_Text sf_admin_form_field_birth_state">
    ...
    <label for="birth_state"> <?php echo __('Provincia de nacimiento');?> </label>
      <?php echo (is_null($student->getPerson()->getBirthState()) | $student->getPerson()->getBirthState() == '') ? 'No ha sido cargado' : $student->getPerson()->getBirthStateRepresentation();?>    </div>
    <div style="margin-top: 1px; clear: both;"></div>
  </div>
  <div class="sf_admin_form_row sf_admin_Text sf_admin_form_field_birth_city">
    ...
    <label for="birth_city"> <?php echo __('Ciudad de nacimiento');?> </label>
      <?php echo (is_null($student->getPerson()->getBirthCity()) | $student->getPerson()->getBirthCity() == '') ? 'No ha sido cargada' : $student->getPerson()->getBirthCityRepresentation();?>    </div>
    <div style="margin-top: 1px; clear: both;"></div>
  </div>
